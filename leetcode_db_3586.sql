...
-- link to problem is : https://leetcode.com/problems/find-covid-recovery-patients/description/
...
FROM
     patients p 
LEFT JOIN covid_tests ct1 ON p.patient_id = ct1.patient_id
...
FROM
     patients p 
LEFT JOIN covid_tests ct1 ON p.patient_id = ct1.patient_id
WHERE ct1.result = 'Negative'
)
SELECT
       pr.patient_id
      ,pr.patient_name
      ,pr.age
      ,ABS(MIN(nr.test_date) - MIN(Pr.test_date)) recovery_time 
FROM
    positive_results pr  
INNER JOIN negative_results nr  ON pr.patient_id = nr.patient_id
WHERE  nr.test_date > pr.test_date
GROUP BY
       pr.patient_id
      ,pr.patient_name
      ,pr.age
ORDER BY recovery_time ASC,  pr.patient_name ASC ;
--End of Solution
