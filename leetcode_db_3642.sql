...
-- link to problem is : https://leetcode.com/problems/find-books-with-polarized-opinions/
...
WITH book_id_ps AS 
(
   SELECT a.book_id, round(((a.polarized + c.polarized )/b.total),2) ps
   FROM
            (SELECT book_id 
                ,count(session_rating)  polarized
...
SELECT
        ...
        ,c.ps polarization_score
FROM
           books b
INNER JOIN reading_sessions rs  ON b.book_id = rs.book_id    
LEFT JOIN  book_id_ps        C  ON b.book_id = c.book_id
WHERE      
      b.book_id IN (    
