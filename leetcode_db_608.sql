...
-- link to problem is : https://leetcode.com/problems/tree-node/
...
SELECT 
     id,
     CASE 
     when p_id is null      and p_level = 1 then 'Root'
     when p_id is not null  and id in (select distinct p_id from tree) then 'Inner'
     when p_id is not null  and id not in (select distinct p_id from tree)  then 'Leaf'
     ELSE 'Leaf'
     END as type
from
(select distinct id, P_id, level p_level
from tree
