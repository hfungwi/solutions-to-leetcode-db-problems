...
-- link to problem is : https://leetcode.com/problems/find-stores-with-inventory-imbalance/
...
WITH most_exp_product AS (
    ...
    FROM
        ...
        INNER JOIN (
            ...
            GROUP BY
                store_id
        ) a ON i.price = a.max_price
               AND i.store_id = a.store_id
), least_exp_product AS (
    SELECT DISTINCT
        a.store_id,
        i.product_name,
        i.price,
    ...
    FROM
        ...
        INNER JOIN (
            ...
            GROUP BY
                store_id
        ) a ON i.price = a.min_price
               AND i.store_id = a.store_id
)
SELECT
    lep.store_id                          AS "store_id",
    s.store_name                          AS "store_name",
    s.location                            AS "location",
    mep.product_name                      AS "most_exp_product",
    lep.product_name                      AS "cheapest_product",
    round(lep.quantity / mep.quantity, 2) AS "imbalance_ratio"
FROM
         least_exp_product lep
    INNER JOIN most_exp_product mep ON lep.store_id = mep.store_id
    LEFT JOIN stores           s    ON lep.store_id = s.store_id
WHERE
    lep.store_id IN (
        SELECT DISTINCT
            store_id
        FROM
            (
                SELECT
                    store_id, COUNT(DISTINCT product_name)
                FROM
                    inventory
                GROUP BY
                    store_id
