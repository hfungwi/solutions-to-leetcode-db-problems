...
-- link to problem is : https://leetcode.com/problems/find-churn-risk-customers/description/
...
WHERE
  ...
  WHERE
            se2.user_id = se1.user_id
    AND se2.event_type <> 'cancel'
    AND se2.monthly_amount > 0
    )
  AND EXISTS (
        SELECT DISTINCT
    user_id
  ...
  AND monthly_amount < (
        SELECT max(monthly_amount) * 0.5
  FROM subscription_events se3
  WHERE
            se3.user_id = se1.user_id
    )
  AND (
        SELECT max(event_date) - min(event_date)
  FROM subscription_events
