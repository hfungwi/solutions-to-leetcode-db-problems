...
-- link to problem is : https://leetcode.com/problems/find-zombie-sessions/
...
SELECT 
       ...
       ,scroll_events as "scroll_count"
FROM
( SELECT
    session_id
   ,user_id
   ,sum(decode(event_type, 'scroll', 1, 0))            scroll_events
   ,sum(decode(event_type, 'purchase', 1, 0))          purchase_events
   ,sum(decode(event_type, 'click', 1, 0))             click_events
   ,(max(event_timestamp) - min(event_timestamp))*1440 duration
FROM
    app_events
 GROUP BY 
          session_id, 
          user_id   
 HAVING
       (max(event_timestamp) - min(event_timestamp))*1440 > 30
   AND sum(decode(event_type, 'purchase', 1, 0)) = 0
)
WHERE
        click_events/scroll_events < 0.20
    AND scroll_events >= 5
ORDER BY
        scroll_events DESC
       ,session_id    ASC
