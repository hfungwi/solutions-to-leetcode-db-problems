...
-- link to problem is : https://leetcode.com/problems/find-books-with-no-available-copies/description/
...
FROM
     library_books lb, borrowing_records br
WHERE 
...
GROUP BY
      ...
      ,lb.genre
      ,lb.publication_year
HAVING   max(lb.total_copies) = count(br.record_id)
ORDER BY
         count(br.record_id) DESC, lb.title ASC 
--End of Solution
