...
-- link to problem is : https://leetcode.com/problems/restaurant-growth/description/
...
FROM 
    (
     SELECT trunc(visited_on) visited_on, 
            sum(amount) amount
     FROM   customer
     GROUP BY trunc(visited_on) 
    ) 
) 
WHERE visited_on IN ( 
                      SELECT DISTINCT trunc(visited_on) 
                      FROM (
                             SELECT          visited_on, 
                                          min(visited_on) OVER (ORDER BY visited_on ASC) AS "first_day",
                                          visited_on - min(visited_on) over (ORDER BY visited_on ASC) diff
                             FROM customer
                          )
                      WHERE DIFF >= 6    
                   )
