...
-- link to problem is : https://leetcode.com/problems/find-drivers-with-improved-fuel-efficiency/
--Start of problem ;
/*
Table: drivers
...
WITH first_half_average as (
SELECT 
        driver_id
        ,avg(distance_km/fuel_consumed) first_half_avg
FROM 
    trips
WHERE
      EXTRACT(MONTH FROM TRIP_DATE) BETWEEN 01 AND 06
GROUP BY
         driver_id
),
second_half_average as (
SELECT 
        driver_id
        ,avg(distance_km/fuel_consumed) second_half_avg
FROM 
    trips
WHERE
      EXTRACT(MONTH FROM TRIP_DATE) BETWEEN 07 AND 12
GROUP BY
         driver_id
)
SELECT 
       ...
       ,round(s.second_half_avg,2) second_half_avg
       ,round(s.second_half_avg - f.first_half_avg, 2) efficiency_Improvement
FROM
           drivers d 
INNER JOIN first_half_average f ON d.driver_id = f.driver_id
INNER JOIN second_half_average s ON d.driver_id = s.driver_id
