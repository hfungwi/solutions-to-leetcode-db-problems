...
-- link to problem is : https://leetcode.com/problems/dna-pattern-recognition/submissions/1940311111/
...
SELECT
       ...
       ,species
       ,case 
              WHEN substr(dna_sequence,1,3) = 'ATG' 
              then 1
              else 0
        end has_start
       ,case
            WHEN substr(dna_sequence,-3) in ('TAA', 'TAG', 'TGA')
               THEN 1
               ELSE 0
        end has_stop
       ...
       ,case
           WHEN REGEXP_LIKE(dna_sequence, 'G{3,}') 
              THEN 1
              ELSE 0
        end has_ggg
FROM
