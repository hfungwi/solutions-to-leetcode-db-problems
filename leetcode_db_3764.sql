...
-- link to problem is :https://leetcode.com/problems/most-common-course-pairs/
...
WITH VALID_USERS AS (
                    ...
                    FROM
                          (  SELECT user_id
                            ...
                            GROUP BY user_id
                            HAVING   
                                count(distinct course_id) >= 5
                                AND avg(course_rating) >= 4
                          )
                 )
,COURSE_ORDER AS (
    SELECT course_name,
           lag(course_name) over (partition by vu.user_id order by completion_date desc) next_course
    FROM valid_users vu
    INNER JOIN course_completions c ON vu.user_id = c.user_id
)
SELECT course_name first_course, next_course second_course, count(1) transition_count
FROM course_order
WHERE next_course is not null
group by course_name, next_course
order by 3 desc, lower(course_name) asc, lower(next_course) asc 
--End of Solution
