...
-- link to problem is : https://leetcode.com/problems/seasonal-sales-analysis/
  ...
  ( 
    WITH BASE_T AS
                  (
                   SELECT 
                     CASE
                         WHEN to_number(to_char(s.sale_date, 'mm')) in (12,1,2)  THEN 'Winter'
                         WHEN to_number(to_char(s.sale_date, 'mm')) in (3,4,5)   THEN 'Spring'
                         WHEN to_number(to_char(s.sale_date, 'mm')) in (6,7,8)   THEN 'Summer'
                         WHEN to_number(to_char(s.sale_date, 'mm')) in (9,10,11) THEN 'Fall'
                      END season
                      ,p.category
    ...
    SELECT
         ...
         ,total_quantity
         ,total_revenue
         ,row_number() over ( partition by season order by total_quantity DESC, total_revenue DESC) RN
    FROM
         BASE_T   
  )
WHERE RN = 1 ;
