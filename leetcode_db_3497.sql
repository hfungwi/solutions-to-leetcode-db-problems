...
-- link to problem is : https://leetcode.com/problems/analyze-subscription-conversion/
...
(SELECT 
    user_id, 
    round(sum(activity_duration)/(count(activity_date)),2) trial_avg_duration
FROM   useractivity
WHERE  activity_type = 'free_trial' 
GROUP BY user_id) A INNER JOIN
(
SELECT  
        ua.user_id, 
        round(sum(activity_duration)/(count(activity_date)),2) paid_avg_duration
