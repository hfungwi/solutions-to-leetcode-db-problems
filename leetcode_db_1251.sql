...
-- link to problem is : https://leetcode.com/problems/average-selling-price/
...
--Start of Solution
select   product_id
        ,nvl(round(sum(price_by_units)/sum(units),2),0) average_price
from        
(select
             p.product_id
            ,p.start_date
            ...
            ,p.price price
            ,u.units units
            ,nvl(p.price,0)*nvl(u.units,0) price_by_units
from  
             prices p
